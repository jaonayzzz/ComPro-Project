...
inline void Custom_Pages(std::vector<Flower>& flowerList,std::vector<Flower>& selectedFlowers,AppState& state,UserSelection& selection){
        ...
        //set ค่าที่รับจาก mainmenu 
        int min = selectedCont.minF;; 
        int max = selectedCont.maxF; 
        int containerPrice = selectedCont.basePrice;
        std::string Containertype = selectedCont.type; 
        std::string Containersize = selectedCont.size;

        if (currentPage == 0){  
            PrintSendYourFeelings();
           
            bool ContainerisFull = (selectedFlowers.size() >= static_cast<size_t>(max)); //ประกาศตัวแปรเช็กภาชนะ
            ImGui::BeginChild("FlowerScrollZone", ImVec2(0, -footerHeight), false, ImGuiWindowFlags_None);
            ImDrawList* draw_list = ImGui::GetWindowDrawList();
            ImVec2 windowPos = ImGui::GetCursorScreenPos();

            //เช็กโอกาสที่เลือกเพื่อเรียงดอกไม้
            if (!selection.occasion.empty()) {
                std::stable_sort(flowerList.begin(), flowerList.end(), [&](const Flower& a, const Flower& b) {
                    bool userList = (a.occasion == selection.occasion);
                    bool originalList = (b.occasion == selection.occasion);
                    
                    // สิ่งที่ user เลือกไม่ตรงกับการเรียงต้นแบบ
                    if (userList && !originalList) return true;
                    return false;// กรณีอื่นๆ ให้คงลำดับเดิมไว้
                });
            }
            ...
            for (int i = 0; i < (int)flowerList.size(); i++) {
                ...
                ImU32 boxBgCol = ContainerisFull ? ImColor(255, 255, 255, 60) : ImColor(255, 255, 255, 150);
                ImU32 borderCol = ContainerisFull ? ImColor(200, 200, 200, 100) : ImColor(255, 255, 255, 255);
                // 1. วาดพื้นหลังกล่อง ---
                ...
                // 2.วาดรูปดอกไม้ (ขยับตำแหน่งขึ้นโดยเว้นโอกาสและราคา)
                ...
                draw_list->AddImage((void*)(intptr_t)flowerList[i].texture.getNativeHandle(), imgPos, imgMax);//วาดรูปโดยดึงจาก flowerList 
                }
                ...
                //แสดงโอกาส
                std::string occasionText = flowerList[i].occasion;
                std::string formOccasion = "[" + flowerList[i].occasion + "]";
                float occasionWidth = ImGui::CalcTextSize(formOccasion.c_str()).x;
                draw_list->AddText(ImVec2(boxPos.x + (boxSize - occasionWidth) / 2.0f, boxPos.y + 132.0f), ImColor(0, 0, 0, 255), formOccasion.c_str());
                ...
                if (ImGui::IsItemHovered()) {
                    draw_list->AddRect(boxPos, boxMax, ImColor(255, 215, 0, 255), 12.0f, 0, 2.0f); //ถ้าอยากให้กลมบางมุม : ImDrawFlags_RoundCornersTopLeft 
                    ImGui::BeginTooltip();
                    ImGui::Dummy(ImVec2(75, 0)); //กว้าง 75, ยาวตามบรรทัดตัวอักษร
                    ImGui::PushTextWrapPos(ImGui::GetFontSize() * 10.0f); // ตัดบรรทัดที่ความกว้างประมาณ 10 ตัวอักษร
            ...
            int totalRows = ((int)flowerList.size() + (columns-1)) / columns; 
        ...
        // คำนวณขอบเขตการแสดงผล
        ImDrawList* drawList = ImGui::GetWindowDrawList();
        ImVec2 winPos = ImGui::GetWindowPos();
        ImVec2 winSize = ImGui::GetWindowSize();

        // คำนวณความกว้างของหน้าต่าง(กำหนดใหเป็น 2/3 ของหน้าต่าง)
        ...
        // แสดงข้อมูล
        // ใช้ CursorPos เพื่อกำหนดให้ ImGui แสดงข้อความเริ่มจากมุมซ้ายบนของสี่เหลี่ยม
        ImGui::SetCursorScreenPos(ImVec2(rectMin.x + textInsidePadding, rectMin.y + textInsidePadding));// มีกาเว้นพื้นที่ด้านบนและด้านซ้าย

        //ส่วนแสดงผล
        ImGui::BeginGroup();
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.1f, 0.1f, 0.3f, 1.0f)); // เพิ่มสี text 
        ...
        float nextLine = 10.0f; // เว้นระยะระหว่างบรรทัด
        float lineY1 = currentCursorPos1.y + nextLine ; // ให้เส้นอยู่ต่ำกว่าตัวหนังสือบรรทัดบนเล็กน้อย
        drawList->AddLine(
        ...
        ImGui::SetCursorScreenPos(ImVec2(currentCursorPos1.x, lineY1 + nextLine));
        ImGui::Text("%s (%s)", Containertype.c_str(), Containersize.c_str(), selectedCont.basePrice);
        std::string baseprice = std::to_string(selectedCont.basePrice) + " THB";// ราคาภาชนะ
        float priceTextWidth = ImGui::CalcTextSize(baseprice.c_str()).x;
        float rightPadding = 20.0f;//ห่างจากขอบขวา
        ...
        // เลื่อน Cursor 
        ImGui::SetCursorScreenPos(ImVec2(currentCursorPos2.x, lineY2 + nextLine));
